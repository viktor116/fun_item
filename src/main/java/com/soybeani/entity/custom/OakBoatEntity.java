...
public class OakBoatEntity extends AnimalEntity {

    public static final EntityType<OakBoatEntity> OAK_BOAT = Registry.register(
            Registries.ENTITY_TYPE,
            Identifier.of(InitValue.MOD_ID, "oak_boat_entity"),
            FabricEntityTypeBuilder.create(SpawnGroup.CREATURE, OakBoatEntity::new)
    ...
    protected OakBoatEntity(EntityType<? extends AnimalEntity> entityType, World world) {
    ...
    private void updatePaddles() {
        ...
        if (this.getVelocity().length() > 0.1) {
            for (int i = 0; i < 2; i++) {
                paddlePhases[i] += 0.3927f; // 大约11.25度
            }
        }
    }
    ...
    public boolean isBreedingItem(ItemStack stack) {
        return stack.isOf(Items.OAK_PLANKS) || stack.isOf(Items.OAK_LOG);
    }
    ...
    public void breed(ServerWorld world, AnimalEntity other) {
        OakBoatEntity baby = (OakBoatEntity) this.createChild(world, other);
        if (baby != null) {
            // 设置婴儿年龄为负数（表示成长时间）
            baby.setBaby(true);
            ...
            ((OakBoatEntity)other).setBreedingAge(600);
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        // 如果是幼年且手持小麦，则加速生长
        if (this.isBaby() && (itemStack.isOf(Items.OAK_LOG) || itemStack.isOf(Items.OAK_PLANKS))) {
            // 加速生长
    ...
    public void dropLoot(DamageSource source, boolean causedByPlayer) {
        ...
        this.dropItem(Items.OAK_BOAT, 1);
    }
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(3, new TemptGoal(this, 1.2,
                Ingredient.ofItems(Items.OAK_LOG,Items.OAK_PLANKS), false));
