 ...
 * @description
 */
public class FlyBoatItem extends Item {
    public FlyBoatItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (!world.getBlockState(blockPos).isAir()) {
                ...
                if (!world.isClient) {
                    FlyBoatEntity boat = new FlyBoatEntity(world,
                            spawnPos.getX() + 0.5D,
                            spawnPos.getY(),
                            spawnPos.getZ() + 0.5D);
...
//                    }
    ...
    public static final Text FLY_BOAT_TOOLTIP = Text.translatable("itemTooltip."+ InitValue.MOD_ID +".fly_boat").formatted(Formatting.WHITE);

    @Override
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add(FLY_BOAT_TOOLTIP);
    }
}
